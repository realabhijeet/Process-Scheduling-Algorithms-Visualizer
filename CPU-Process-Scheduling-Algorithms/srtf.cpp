...
void srtf(const vector<int>& arrival_time, const vector<int>& burst_time) {
    ...
    while (completed_count < n) {
        ...
        for (int i = 0; i < n; ++i) {
            if (arrival_time[i] <= current_time && !completed[i] && remaining_time[i] < min_remaining_time) {
        ...
        if (remaining_time[shortest_index] == 0) {
            ...
            ++completed_count;
            turnaround_time[shortest_index] = current_time + 1 - arrival_time[shortest_index];
